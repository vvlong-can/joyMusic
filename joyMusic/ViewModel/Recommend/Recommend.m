...
-(void)fetchValueSuccessWithDic: (NSDictionary *) returnValue
{
    ...
    for (int i = 0; i < statuses.count; i ++) {
        RecommendMusicModel *recommendMusic = [[RecommendMusicModel alloc] init];
        ...
        recommendMusic.date = [resultFormatter stringFromDate:date];
        recommendMusic.userName = statuses[i][USER][USERNAME];
        recommendMusic.text = statuses[i][WEIBOTEXT];
        recommendMusic.imageUrl = [NSURL URLWithString:statuses[i][USER][HEADIMAGEURL]];
        recommendMusic.userId = statuses[i][USER][UID];
        recommendMusic.weiboId = statuses[i][WEIBOID];
        
        [publicModelArray addObject:recommendMusic];
